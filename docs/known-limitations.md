...
| 编号 | 现象 | 影响 | 位置 | 改进方向 |
|---|---|---|---|---|
| **L11** | **模型自报的 confidence 无信息量** —— 2026-08-03 实测 glm-4.5-flash 在 5 个案例(含判断方向相反的对抗例)上**一律返回 1.0** | `min_confidence=0.70` 这道闸门实际**没有拦下任何东西**;承重的是另外三道确定性控制(数值不漂移 / 极性 / 引文锚定) | `audit/semantic_control.py` | 别指望 confidence:改用自洽性采样(同一问题多 seed 看是否稳定),或干脆去掉这道闸门。**每次运行打印阈值敏感度曲线**,让读者自己看出闸门是否在做事 |
