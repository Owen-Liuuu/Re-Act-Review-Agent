...
- ``population_phrase``: the paper's OWN words for which people this counts
  ...
  "received at least one dose"). If those words are not inside ``quote``, put
  the passage that carries them in ``population_quote`` — again one contiguous
  verbatim passage. Leave both empty if the paper does not say; an empty field
  is an honest answer and a guessed one is not.
...
{{"readings": [{{{reading_fields}
    ...
    "population_quote": "passage carrying those words, if not in quote",
...
def build_batch_prompt(
    *, target_shape: str, context: str, concept: str, raw_label: str,
    ...
    return (_HEADER.format(context=context or "a systematic review",
            ...
            + _RULES.format(paper_text=paper_text,
